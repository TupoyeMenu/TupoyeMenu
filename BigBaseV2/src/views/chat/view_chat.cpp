...
namespace big
{
    ...
    void view::chat()
    {
		...
		while (clipper.Step())
		{
			...
			{
				if (!g_chat_service->get_msgs()[i].is_spam)
				{
					CNetGamePlayer* net_player = g_chat_service->get_msgs()[i].player;
					std::string name = net_player->get_name();
					std::string_view msg = g_chat_service->get_msgs()[i].msg;
					bool is_team = g_chat_service->get_msgs()[i].is_team;
					ImGui::TextWrapped("%s [%s]  %s", name.c_str(), is_team ? "local" : "all", msg.data());
				}
			}
		}
		...
		components::input_text_with_hint("###Message", "Message", &g->chat.message, input_text_flags, [&reclaim_focus]
		{
			...
			g_chat_service->add_msg(g_player_service->get_self()->get_net_game_player(), g->chat.message, g->chat.local, false);
