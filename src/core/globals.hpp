	...
	public:
		...
		struct session
		{
			...
			NLOHMANN_DEFINE_TYPE_INTRUSIVE(session,
				local_weather, override_time, override_weather, custom_time, population_control, chat_force_clean, log_chat_messages,
		...
		struct settings
		{
			struct hotkeys
			{
				...
				NLOHMANN_DEFINE_TYPE_INTRUSIVE(hotkeys, editing_menu_toggle, menu_toggle, teleport_waypoint, teleport_objective, 
					...
					invisveh, localinvisveh, object_spooner, object_spooner_ctx)
			} hotkeys{};
		...
		struct spoofing
		{
			...
			NLOHMANN_DEFINE_TYPE_INTRUSIVE(spoofing,
				spoof_username, spoof_local_username, username, spoof_ip, ip_address, spoof_rockstar_id, rockstar_id, pool_type, spoof_cheater,
