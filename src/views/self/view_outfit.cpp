	...
	void view::outfit_editor(Ped ped_to_edit)
	{
		...
		if (ImGui::InputInt("Drawable Id", &g.outfit_editor.drawableId))
		{
			g_fiber_pool->queue_job([ped_to_edit] {
				PED::SET_PED_COMPONENT_VARIATION(ped_to_edit,
				    g.outfit_editor.componentId,
				    g.outfit_editor.drawableId,
				    g.outfit_editor.textureId,
				    g.outfit_editor.paletteId);
			});
		}
		...
		components::button("Test component", [ped_to_edit] {
			...
			Hash hash = FILES::GET_HASH_NAME_FOR_COMPONENT(ped_to_edit,
			    g.outfit_editor.componentId,
			    g.outfit_editor.drawableId,
			    g.outfit_editor.textureId);
		...
		{
			g_fiber_pool->queue_job([ped_to_edit] {
				PED::SET_PED_PROP_INDEX(ped_to_edit,
				    g.outfit_editor.propId,
				    g.outfit_editor.propDrawableId,
				    g.outfit_editor.propTextureId,
				    true,
				    0);
			});
		}
