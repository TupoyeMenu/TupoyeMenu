...
namespace big
{
	void view::outfit_slots()
	{
		if (*g_pointers->m_gta.m_script_globals)
		{
			...
			{
				ImGui::SetNextItemWidth(120);
				ImGui::SliderInt(std::format("{} {} [0,{}]##2", item.label, "TEX", item.texture_id_max).c_str(), outfit::get_component_texture_id_address(slot, item.id), 0, item.texture_id_max);
			}
			ImGui::EndGroup();
			...
			{
				ImGui::SetNextItemWidth(120);
				ImGui::SliderInt(std::format("{} {} [0,{}]##4", item.label, "TEX", item.texture_id_max).c_str(), outfit::get_prop_texture_id_address(slot, item.id), 0, item.texture_id_max);
			}
			ImGui::EndGroup();
		}
	}
