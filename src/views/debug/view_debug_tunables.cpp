...
namespace big
{
	...
	std::string get_tunable_display(tunable_debug& tunable_test)
	{
		...
		return "Invalid Tunable data read.";
	}
	...
	void view::debug_tunables()
	{
		static tunable_debug tunable_test{};
		ImGui::SetNextItemWidth(300.f);
		components::input_text("Tunable", tunable_test.tunable_name);
		if (auto ptr = get_tunable_ptr(tunable_test))
		{
			...
			ImGui::SetNextItemWidth(300.f);
			ImGui::InputScalar("Offset", ImGuiDataType_S32, &tunable_offset, 0, 0, 0, ImGuiInputTextFlags_ReadOnly);
			...
			case TunableValueType::BOOLEAN:
			{
				...
				if (ImGui::Checkbox("Value", &is_tunable_enabled))
				{
		...
		{
			ImGui::Text("Invalid Tunable data read.");
		}

		ImGui::SetNextItemWidth(150.f);
		ImGui::Combo("Type", (int*)&tunable_test.tunable_value_type, "INT\0BOOLEAN\0BITSET\0FLOAT\0");

		auto tunables = list_tunables();
		static std::string selected_tunable;
		ImGui::Text("Saved Tunables");
