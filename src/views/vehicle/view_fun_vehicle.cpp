...
namespace big
{
	void view::fun_vehicle(Vehicle vehicle_to_edit)
	{
		components::sub_title("Seat Changer");
		{
			...
			if (vehicle_to_edit != 0 && ready == true)
			{
				ready = false;

				g_fiber_pool->queue_job([vehicle_to_edit] {

					std::map<int, bool> tmp_seats;

					int num_of_seats = VEHICLE::GET_VEHICLE_MAX_NUMBER_OF_PASSENGERS(vehicle_to_edit);

					for (int i = -1; i < num_of_seats; i++)
					{
						tmp_seats[i] = VEHICLE::IS_VEHICLE_SEAT_FREE(vehicle_to_edit, i, true);
					}
			...
			else
			{
				for (auto& it : seats)
				{
					...
					components::button(name, [idx, vehicle_to_edit] {
						PED::SET_PED_INTO_VEHICLE(self::ped, vehicle_to_edit, idx);
					});
					if (!it.second)
					{
						ImGui::EndDisabled();
