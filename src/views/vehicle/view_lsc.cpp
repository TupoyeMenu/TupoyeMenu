...
namespace big
{
	void view::lsc()
	{
			...
			g_fiber_pool->queue_job([] {
				...
				for (int slot = MOD_SPOILERS; slot <= MOD_LIGHTBAR; slot++)
				{
					...
					if (count > 0)
					{
						...
						std::string slot_name = vehicle::get_mod_slot_name(model, player_vehicle, slot);
						if (slot_name.empty())
						{
							continue;
						}
						...
						for (int mod = -1; mod < count; mod++)
						{
							...
							std::string mod_name = vehicle::get_mod_name(model, player_vehicle, slot, mod, count);
		...
		ImGui::SameLine();
		if (components::button("Max Vehicle Performance"))
		{
			g_fiber_pool->queue_job([] {
		...
		{
			if (ImGui::Checkbox("Clan Logo", &has_clan_logo))
			{
				g_fiber_pool->queue_job([] {
					if (has_clan_logo)
					{
					...
					if (ImGui::Selectable(name.c_str(), item_selected))
					{
						g_fiber_pool->queue_job([&mod, is_wheel_mod, wheel_stock_mod, wheel_custom] {
							...
							if (selected_slot >= 0)
							{
								...
								{
									g_notification_service.push_error("LS Customs", "Selected mod is invalid.");
								}
							}
