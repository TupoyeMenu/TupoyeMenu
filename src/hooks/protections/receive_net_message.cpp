...
namespace big
{
	...
	bool should_block_script(std::uint32_t hash, const menu_settings::script_block_opts& opts)
	{
		...
		return (opts.lsc && hash == "carmod_shop"_J) || (opts.ammunation && hash == "gunclub_shop"_J)
		    || (opts.arcade_games && arcade_games.contains(hash)) || (opts.casino_games && casino_games.contains(hash)) || (opts.drones && hash == "am_mp_drone"_J) || (opts.impromptu_race && hash == "mg_race_to_point"_J)
		    || (opts.interiors && interiors.contains(hash)) || (opts.movies && hash == "act_cinema"_J)
		    || (opts.prostitutes && prostitutes.contains(hash)) || (opts.sitting && sitting.contains(hash))
		    || (opts.sleeping && sleeping.contains(hash)) || (opts.stores && stores.contains(hash)) || (opts.street_dealer && hash == "fm_street_dealer"_J)
		    || (opts.strip_club && strip_club.contains(hash)) || (opts.atms && hash == "atm_trigger"_J) || (opts.impromptu_dm && hash == "fm_impromptu_dm_controler"_J) || (opts.gang_attacks && hash == "fm_hideout_controler"_J)
	...
	bool hooks::receive_net_message(void* a1, rage::netConnectionManager* net_cxn_mgr, rage::netEvent* event)
	{
		...
		case rage::eNetMessage::MsgScriptJoin:
		{
			...
			if (player && handle_block_script(player, script, event->m_msg_id))
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "Denying script request from {} (hash={:X}, instance={})",
				    player->get_name(),
				    script.m_hash,
				    script.m_instance_id);
				return true;
			}
		...
		case rage::eNetMessage::MsgRadioStationSyncRequest:
		{
			...
			if (!session || !session->is_host())
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRadioStationSyncRequest, but we are not the host",
				    peer->m_info.name);
				return true;
			}
		...
		case rage::eNetMessage::MsgRequestObjectIds:
		{
			...
			if (!gta_util::get_net_object_ids())
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRequestObjectIds, but we don't have a valid CNetworkObjectMgr",
				    peer->m_info.name);
				return true;
			}
			...
			if (player->received_object_id_request)
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRequestObjectIds, but we've already received a request from them",
				    peer->m_info.name);
				return true;
			}
		...
		case rage::eNetMessage::MsgInformObjectIds:
		{
			...
			if (!gta_util::get_net_object_ids())
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgInformObjectIds, but we don't have a valid CNetworkObjectMgr",
				    peer->m_info.name);
				return true;
			}

			if ((gta_util::get_net_object_ids()->m_object_id_response_pending_players & (1 << player->id())) == 0
			    && player->received_object_id_response)
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgInformObjectIds, but we didn't request them for it",
				    peer->m_info.name);
				return true;
			}
			...
			if (num_objects_in_our_range > 256)
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgInformObjectIds, but they have given us an unusual amount of occupied object IDs in our object range",
				    peer->m_info.name);
			...
			for (int i = 0; i < num_replacement_objects; i++)
			{
				...
				if ((*g_pointers->m_gta.m_network_object_mgr)->find_object_by_id(object_id, true))
				{
					LOGF(stream::net_messages,
					    WARNING,
					    "{} sent MsgInformObjectIds, but they have given us an object ID that is not actually free",
					    peer->m_info.name);
					gta_util::get_net_object_ids()->m_object_id_response_pending_players &= (1 << player->id());
					return true;
				}
		...
		case rage::eNetMessage::MsgRoamingJoinBubbleAck:
		{
			...
			if (status == 0 && bubble == 10)
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRoamingJoinBubbleAck with a null bubble id",
				    peer->m_info.name);
				if (player)
			...
			else if (status == 0)
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} wants us to join their bubble {}, but this is not a good idea",
				    peer->m_info.name,
				    bubble);
				return true;
			}
		...
		case rage::eNetMessage::MsgRoamingInitialBubble:
		{
			...
			if (player && g_player_service->get_self()->id() != -1)
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRoamingInitialBubble, but the host has already joined (and so have we)",
				    peer->m_info.name);
				return true;
			}
			...
			if (their_bubble == 10) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRoamingInitialBubble, but the host doesn't have a bubble?",
				    peer->m_info.name);
				return true;
			}
			...
			if (my_bubble == 10) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRoamingInitialBubble, but the host didn't actually give us a valid bubble",
				    peer->m_info.name);
				return true;
			}
			...
			if (my_bubble > 10 || their_bubble > 10) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRoamingInitialBubble, but the host is trying to crash us by giving us an out of bounds bubble id",
				    peer->m_info.name);
				return true;
			}
			...
			if (my_bubble != 0) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRoamingInitialBubble with a non-standard bubble id: {}",
				    peer->m_info.name,
				    my_bubble);
			}
			...
			if (my_bubble != their_bubble) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRoamingInitialBubble, but the host's bubble id doesn't match our bubble id ({} != {})",
				    peer->m_info.name,
				    their_bubble,
				    my_bubble);
				return true;
			...
			if (my_pid >= 32 || their_pid >= 32) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRoamingInitialBubble, but the host gave us invalid player ids (or made us pick our own player ids)",
				    peer->m_info.name);
			}
			...
			if (my_pid == their_pid) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgRoamingInitialBubble, but the host has the same player id as us",
				    peer->m_info.name);
				return true;
			}
		...
		case rage::eNetMessage::MsgNonPhysicalData:
		{
			...
			if (bubble_id == 10) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    VERBOSE,
				    "{} sent MsgNonPhysicalData and indicated that they are not in a bubble",
				    peer->m_info.name);
			...
			if (bubble_id > 10) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgNonPhysicalData, but are trying to crash us by giving us an out of bounds bubble id",
				    peer->m_info.name);
				return true;
			}
			...
			if (bubble_id != 0) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgNonPhysicalData with a non-standard bubble id: {}. This may cause problems during join",
				    peer->m_info.name,
				    bubble_id);
			}
			...
			if (g_player_service->get_self() && g_player_service->get_self()->id() != -1 && g_player_service->get_self()->id() == player_id) [[unlikely]]
			{
				LOGF(stream::net_messages,
				    VERBOSE,
				    "{} sent MsgNonPhysicalData, but are trying to replace us",
				    peer->m_info.name);
				return true;
			}
			...
			for (auto& player : g_player_service->players())
			{
				if (player.second->id() == player_id) [[unlikely]]
				{
					LOGF(stream::net_messages,
					    VERBOSE,
					    "{} sent MsgNonPhysicalData, but are trying to replace {}",
					    peer->m_info.name,
		...
		case rage::eNetMessage::MsgConfigRequest:
		{
			LOGF(stream::net_messages,
			    WARNING,
			    "Denying MsgConfigRequest from {} ({})",
			    peer->m_info.name,
			    peer->m_info.handle.m_rockstar_id);
			return true;
		}
		...
		case rage::eNetMessage::MsgTextMessage2:
		{
			...
			if (!player)
			{
				LOGF(stream::net_messages,
				    WARNING,
				    "{} sent MsgTextMessage, but is not physical yet. This may indicate chat spam",
				    peer->m_info.name);
				return true;
			}
			...
			else
			{
				...
				if (g.session.chat_commands && message[0] == g.session.chat_command_prefix)
					...
					g_lua_manager->trigger_event<menu_event::ChatMessageReceived>(player->id(), message, true, is_team);
#endif
