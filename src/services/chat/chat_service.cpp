...
namespace big
{
	...
	{
		m_chat_msgs.push_back({rid, is_team, is_spam, name, msg});
	}
	void chat_service::add_msg(CNetGamePlayer* player, std::string msg, bool is_team, bool is_spam)
	{
		m_chat_msgs.push_back({player->get_net_data()->m_gamer_handle.m_rockstar_id, is_team, is_spam, player->get_name(), msg});
	}
	...
	{
		m_direct_msgs.push_back({rid_sender, rid_receiver, is_spam, name_sender, name_receiver, msg});
	}
	void chat_service::add_direct_msg(CNetGamePlayer* player_sender, CNetGamePlayer* player_receiver, std::string msg, bool is_spam)
	{
		m_direct_msgs.push_back({player_sender->get_net_data()->m_gamer_handle.m_rockstar_id,
