...
namespace big
{
	class tp_to_player : player_command
	{
		...
		virtual std::optional<command_arguments> parse_args(const std::vector<std::string>& args, const std::shared_ptr<command_context> ctx) override
		{
			...
			{
				g_notification_service.push_error(std::string("Teleport to Player"), std::string("Invalid player name(s)"));
				return std::nullopt;
			}
		...
		virtual void execute(player_ptr player, const command_arguments& _args, const std::shared_ptr<command_context> ctx) override
		{
			...
			if (target && target->get_ped() && target->get_ped()->get_position())
			{
				...
				g_notification_service.push("Teleport to Player", std::format("Teleporting {} to {}", sender_name, target_name));
			}
		}
	};
