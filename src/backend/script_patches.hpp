...
namespace big
{
	void register_script_patches()
	{
		...
		g_script_patcher_service->add_patch({RAGE_JOAAT("freemode"),
		    "prevent normal blip update",
		    "06 56 ? ? 38 02 2C ? ? ? 71 71",
		    0,
		    {0x2B, 0x55},
		    &g.spoofing.spoof_blip}); // prevent normal blip update
		g_script_patcher_service->add_patch({RAGE_JOAAT("freemode"),
		...
		g_script_patcher_service->add_patch({RAGE_JOAAT("carmod_shop"), "allow all vehicles 2", "2D 03 07 00 00 71 38 02", 5, {0x72, 0x2E, 0x03, 0x01}, nullptr}); // allow all vehicles 2
